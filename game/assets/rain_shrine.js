// rain_shrine.js: Wind & Rain building, generated as code (no imports, no textures). Candidate rain_shrine_d.
export default function (THREE) {
  ...
  // Rain Shrine D: pool in front open to the sky, small four-post thatch canopy at the back
  // sheltering a tall four-faced idol (gable open to the front), carved poles flanking the
  // pool with a line of linen/team strips, bowls on the kerb.
  // stone platform
  cyl(2.3, 2.4, 0.16, 'stone', 0, 0.08, 0, 10);
  for (let i = 0; i < 12; i++) { const t = i / 12 * PI * 2; box(0.9, 0.24, 0.55, 'stone', Math.sin(t) * 2.2, 0.1, Math.cos(t) * 2.2, 0, t + J(0.1), 0); }
  // basin
  const bz = 0.85;
  for (let i = 0; i < 10; i++) { const t = i / 10 * PI * 2; box(0.6, 0.5 + J(0.06), 0.34, 'stone', Math.sin(t) * 0.98, 0.4, bz + Math.cos(t) * 0.98, J(0.05), t, J(0.05)); }
  cyl(0.92, 0.92, 0.08, 'water', 0, 0.56, bz, 12);
  for (const t of [0.5, -0.7, 2.2]) bowl(g, Math.sin(t) * 0.98, 0.66, bz + Math.cos(t) * 0.98, 0.15, 'timD');
  // canopy
  const SX = 1.25, Z0 = -2.0, Z1 = -0.35, yW = 2.95, zc = (Z0 + Z1) / 2;
  const C = grp(0, 0, zc);
  const hz = (Z1 - Z0) / 2;
  for (const sx of [-1, 1]) for (const sz of [-1, 1]) { cyl(0.14, 0.17, yW, 'timD', sx * SX, yW / 2, sz * hz, 6, 0, 0, J(0.03), C); box(0.4, 0.3, 0.4, 'stone', sx * SX, 0.3, sz * hz, 0, 0, 0, C); }
  for (const sz of [-1, 1]) box(2 * SX + 0.5, 0.24, 0.24, 'timD', 0, yW - 0.05, sz * hz, 0, 0, 0, C);
  for (const sx of [-1, 1]) box(0.24, 0.24, 2 * hz + 0.5, 'timD', sx * SX, yW - 0.2, 0, 0, 0, 0, C);
  const R = roof(C, { w: 2 * SX, l: 2 * hz, yW, rise: 1.2, over: 0.4, oz: 0.35, th: 0.24, courses: 2, step: 0.12, rolls: true });
  horses(C, R, hz + 0.37, 'timD', 0.4, 0.18);
  for (let i = 0; i < 6; i++) { const L = 0.5 + rnd() * 0.4; box(0.14, L, 0.03, i % 2 ? 'team' : 'linen', -1.1 + i * 0.44, yW - 0.15 - L / 2, hz + 0.14, J(0.1), 0, J(0.08), C); }
  // back wattle screen so the rear reads
  for (let i = 0; i < 6; i++) box(0.36, 1.6, 0.08, 'timD', -1.0 + i * 0.4, 0.95, -hz - 0.1, 0, 0, J(0.04), C);
  // idol
  box(0.8, 0.3, 0.8, 'stone', 0, 0.3, zc, 0, 0.3, 0);
  idol(g, 0, 0.45, zc, 4, 0.5, 0.56);
  // front carved poles with strip line
  for (const s of [-1, 1]) cpole(g, s * 1.95, 1.3, 3.4, 0.13);
  rod([-1.95, 2.7, 1.3], [1.95, 2.8, 1.3], 0.025, 'linen', 4);
  for (let i = 0; i < 7; i++) box(0.13, 0.45 + rnd() * 0.35, 0.02, i % 2 ? 'team' : 'linen', -1.55 + i * 0.52, 2.45, 1.3, J(0.1), 0, J(0.06));
  for (const s of [-1, 1]) cpole(g, s * 1.95, -1.4, 2.4, 0.1);
