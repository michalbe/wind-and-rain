// khata.js: Wind & Rain building, generated as code (no imports, no textures). Candidate khata_a.
export default function (THREE) {
  ...
  // Khata A: log cabin, gable front with door, white daub gable, three-course thatch.
  const W = 3.6, D = 3.5, y0 = 0.28;
  ...
  for (const s of [-1, 1]) { // gable half-timber
    bar([0, yW, s * (D / 2 + 0.12)], [0, yW + rise * 0.9, s * (D / 2 + 0.12)], 0.16, 0.08, 'timD');
    box(W * 0.85, 0.16, 0.1, 'timD', 0, yW + 0.05, s * (D / 2 + 0.12));
  }
  horses(g, R, D / 2 + R.zf + 0.03, 'timD', 0.5, 0.22);
  horses(g, R, -D / 2 - R.zb - 0.03, 'timD', 0.5, 0.22);
  // door
  for (const s of [-1, 1]) box(0.2, 1.85, 0.26, 'timD', s * 0.58, y0 + 0.92, D / 2 + 0.04);
  box(1.5, 0.28, 0.3, 'timD', 0, y0 + 1.86, D / 2 + 0.06);
  for (let i = 0; i < 3; i++) box(0.31, 1.62, 0.06, 'timD', -0.32 + i * 0.32, y0 + 0.81, D / 2 - 0.1, 0, 0, J(0.02));
  doorCloth(g, 0, y0 + 2.02, D / 2 + 0.12, 1.25);
  box(1.1, 0.2, 0.55, 'stone', 0, 0.1, D / 2 + 0.45, 0, J(0.1), 0);
  // daub patches between logs on the sides and back
  for (const s of [-1, 1]) { patch(g, 'x', s, s * (W / 2 + 0.12), 1.2, -0.6 * s, 0.9, 0.5); patch(g, 'x', s, s * (W / 2 + 0.12), 1.9, 0.8 * s, 0.6, 0.35); }
  patch(g, 'z', -1, -D / 2 - 0.13, 1.0, 0.9, 1.0, 0.45); patch(g, 'z', -1, -D / 2 - 0.13, 1.8, -1.0, 0.6, 0.35);
  // side window with shutter
  box(0.1, 0.6, 0.7, 'timD', W / 2 + 0.18, 1.5, 0.3); box(0.1, 0.12, 1.0, 'timD', W / 2 + 0.2, 1.85, 0.3);
  // bench and woodpile
  box(1.3, 0.1, 0.35, 'timL', -1.1, 0.5, D / 2 + 0.5); for (const s of [-1, 1]) box(0.12, 0.45, 0.3, 'timD', -1.1 + s * 0.5, 0.22, D / 2 + 0.5);
  for (let i = 0; i < 3; i++) for (let j = 0; j < 3 - i; j++) logZ(1.1, 0.12, 'timL', -W / 2 - 0.35 - j * 0.24 + i * 0.12, 0.12 + i * 0.21, -0.5);
