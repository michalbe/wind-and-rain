// war_hall.js: Wind & Rain building, generated as code (no imports, no textures). Candidate war_hall_b.
export default function (THREE) {
  ...
  // War Hall B: tall central gable hall (ridge along z, gable front with door)
  // flanked by two open lean-to sheds holding weapon racks.
  const CW = 4.4, D = 5.4, h = 3.2;
  frameWall(g, -CW / 2, D / 2, CW / 2, D / 2, 0, h, 3, { door: 1 });
  frameWall(g, CW / 2, -D / 2, -CW / 2, -D / 2, 0, h, 3);
  frameWall(g, -CW / 2, -D / 2, -CW / 2, D / 2, 0, h, 3, { brace: false });
  frameWall(g, CW / 2, D / 2, CW / 2, -D / 2, 0, h, 3, { brace: false });
  const rise = 2.95, yW = h - 0.05;
  const R = roof(g, { w: CW, l: D, yW, rise, over: 0.5, oz: 0.6, th: 0.42, courses: 3, lip: 0.25, rolls: true });
  for (const s of [-1, 1]) { gablePlanks(g, CW, s * (D / 2 + 0.1), yW, rise, 'timL'); horses(g, R, s * (D / 2 + 0.62), 'timD', 0.85, 0.3); }
  trophy(g, 0, yW + 0.9, D / 2 + 0.35, 1.15);
  // lean-to sheds
  const LW = 1.55, lh = 2.0;
  for (const s of [-1, 1]) {
    const xo = s * (CW / 2 + LW);
    for (const z of [-D / 2 + 0.2, 0, D / 2 - 0.2]) { box(0.4, lh, 0.4, 'timD', xo, lh / 2, z, 0, 0, J(0.03)); }
    box(0.44, 0.4, D + 0.4, 'timD', xo, lh, 0);
    frameWall(g, xo, -D / 2 + 0.2, s * CW / 2, -D / 2 + 0.2, 0, lh, 1, { brace: false });
    // mono-pitch thatch from under main eave down to the shed posts
    const a = Math.atan2(2.9 - lh, LW + 0.5), Ls = (LW + 1.2) / Math.cos(a);
    { // two stepped courses: upper dark, lower light, upper one kicked out over the lower
      const cx = s * (CW / 2 + (LW + 0.6) / 2), cy = (2.75 + lh) / 2 + 0.3, dx = s * Math.cos(a), dy = -Math.sin(a), nx = s * Math.sin(a), ny = Math.cos(a);
      box(Ls / 2 + 0.3, 0.36, D + 0.9, 'thatchS', cx - dx * Ls / 4 + nx * 0.14, cy - dy * Ls / 4 + ny * 0.14, 0, 0, 0, -s * (a + 0.12));
      box(Ls / 2 + 0.1, 0.36, D + 1.0, 'thatch', cx + dx * Ls / 4, cy + dy * Ls / 4, 0, 0, 0, -s * a);
    }
    cyl(0.24, 0.24, D + 0.95, 'thatchS', s * (CW / 2 + LW + 0.65), lh + 0.2, 0, 6, PI / 2);
    rack(g, s * (CW / 2 + LW / 2 + 0.1), D / 2 + 0.75, 1.5, 0, 1);
    rack(g, s * (CW / 2 + LW / 2 + 0.1), -0.6, 1.3, 0, 1);
    for (let i = 0; i < 2; i++) cyl(0.25, 0.28, 0.5, 'timL', s * (CW / 2 + LW - 0.3), 0.25, -1.9 + i * 0.6, 7);
  }
  for (const x of [-1.2, 1.2]) { cyl(0.42, 0.42, 0.07, 'team', x, 1.8, -D / 2 - 0.35, 10, PI / 2, 0, 0); sph(0.1, 'iron', x, 1.8, -D / 2 - 0.4, 6, 4); }
  box(1.4, 0.2, 0.9, 'timD', 0, 0.1, D / 2 + 0.6);
