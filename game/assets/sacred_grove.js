// sacred_grove.js: Wind & Rain building, generated as code (no imports, no textures). Candidate sacred_grove_b.
export default function (THREE) {
  ...
  // Sacred Grove B: two great twisted trees at the back leaning into an arch, a stone
  // trilithon gate at the front, low ring of boulders, ribbon poles, skull pole, fire-stone.
  cyl(4.0, 4.4, 0.22, 'stone', 0, 0.11, 0, 12);
  for (let i = 0; i < 14; i++) {
    const t = 0.45 + i / 13 * (PI * 2 - 0.9);
    const s = sph(0.42 + rnd() * 0.2, 'stone', Math.sin(t) * 4.0, 0.25, Math.cos(t) * 4.0, 6, 4); s.scale.set(1.2, 0.8 + rnd() * 0.4, 1);
  }
  for (const t of [PI * 0.62, PI * 1.38, PI * 0.95, PI * 1.05]) standing(g, Math.sin(t) * 3.6, Math.cos(t) * 3.6, 2.2 + rnd() * 0.6, 0.8);
  // trilithon gate at front
  for (const s of [-1, 1]) box(0.75, 2.6, 0.6, 'stone', s * 1.2, 1.3, 3.8, 0, J(0.1), J(0.04));
  box(3.3, 0.55, 0.75, 'stone', 0, 2.85, 3.8, 0, 0, J(0.04));
  for (let i = 0; i < 5; i++) box(0.13, 0.7 + rnd() * 0.3, 0.03, i % 2 ? 'team' : 'linen', -0.9 + i * 0.45, 2.2, 4.2, 0, 0, J(0.1));
  const t1 = tree(g, -1.9, -2.2, 7.0, 0.36, 'bark', 'moss', 4, ['linen', 'team']);
  t1.rotation.z = -0.1;
  const t2 = tree(g, 1.9, -2.1, 6.6, 0.33, 'bark', 'moss', 4, ['team', 'linen']);
  t2.rotation.z = 0.1;
  tree(g, -3.0, 1.4, 4.4, 0.22, 'bark', 'moss', 3);
  ribbonPole(g, 2.9, 1.5, 3.2, ['linen', 'team', 'linen']);
  skullPole(g, -1.9, 2.4, 2.5);
  firestone(g, 0, 0.3);
